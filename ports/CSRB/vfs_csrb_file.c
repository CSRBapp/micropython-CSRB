...
#if MICROPY_VFS && MICROPY_VFS_CSRB
...
STATIC mp_obj_t file_open(fs_user_mount_t *vfs, const mp_obj_type_t *type, mp_arg_val_t *args) {
    ...
    assert(vfs != NULL);
    FRESULT res = f_open(&vfs->csrbfs, &o->fp, fname, mode);
    if (res != FR_OK) {
...
STATIC mp_obj_t csrbfs_builtin_open_self(mp_obj_t self_in, mp_obj_t path, mp_obj_t mode) {
#if 0
    ...
    return file_open(self, &mp_type_vfs_csrb_textio, arg_vals);
#endif
}
MP_DEFINE_CONST_FUN_OBJ_3(csrb_vfs_open_obj, csrbfs_builtin_open_self);

#endif // MICROPY_VFS && MICROPY_VFS_CSRB
